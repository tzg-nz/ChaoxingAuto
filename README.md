...
操作流程：下载代码后，进入下载的文件夹内，然后右键在终端打开（总之终端路径在这个文件夹内即可），然后运行命令 python chaoxing.py 账号名 密码 第几个课程

说明：脚本内置大模型自动答题。视频/图文/测验全部自动处理，**客观题（单选/多选/判断）由大模型作答，简答/填空类主观题一律不作答、留空直接交卷**（详见下面「答题策略」）。默认提交一次即收工不追求满分；需要满分用 `full_score=True`，答错自动重做，3 轮仍失败才把错题存入错题报告文件夹供人工复盘。
...
- **视频刷课**：按 1 倍速真实时间挂机，支持断点续播（接着上次的进度继续）
- **图文完成**：图文任务自动完成
...
# 只做某一场考试（exam_id 是考试地址里 examId= 后那串数字）
python chaoxing.py <用户名> <密码> <课程序号index> <exam_id>
```
...
| `no_submit` | bool | False | True 时考试只作答保存、**不自动交卷**：答案留在服务端，你可以登录网页自己检查后手动交。交卷才有成绩，慎用 |
