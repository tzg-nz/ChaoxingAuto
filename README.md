...
操作流程：下载代码后，进入下载的文件夹内，然后右键在终端打开（总之终端路径在这个文件夹内即可），然后运行命令
...
说明：脚本内置大模型自动答题。视频/图文/测验全部自动处理，**客观题（单选/多选/判断）由大模型作答，简答/填空类主观题一律不作答、留空直接交卷**（详见下面「答题策略」）。默认提交一次即收工不追求满分，命令行是刷全部的逻辑，如果考试已经考过，或者只需要刷考试需见下方详情
