...
**解释**：第一个命令用于下载代码需要的依赖（只需要运行一次即可，后续刷课不需要运行），第二个命令用来打印所有课程，在前方会有对应的序号，然后记住你要刷的那个课程对应的序号运行第三个命令（第几个课程就是对应那个序号）
...
如果出现类似报错大概率是因为你超星的左边侧边栏写的不是课程或者结构不太一样，正常是长这样的（见下图），因为我自己账号是这样的所以出现这个错误暂时没办法解决，要么你就把问题丢给ai让他来修改，要么加我好友远程来修改问题
